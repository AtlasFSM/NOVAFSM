...
describe('TimeEntriesService', () => {
  ...
  const mockPrisma = {
    timeEntry: { create: jest.fn(), findMany: jest.fn(), findFirst: jest.fn(), update: jest.fn(), count: jest.fn() },
  ...
  it('should create time entry', async () => {
    ...
    mockPrisma.timeEntry.create.mockResolvedValue({ id: 't1', jobId: 'j1', startTime: new Date(), endTime: null });
    const result = await service.create('tenant1', { jobId: 'j1', technicianId: 'tech1', startTime: new Date() });
  ...
  it('should calculate duration', async () => {
    ...
    mockPrisma.timeEntry.update.mockResolvedValue({ id: 't1', startTime: start, endTime: end, duration: 8 * 3600 });
