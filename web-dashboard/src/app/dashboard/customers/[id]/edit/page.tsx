...
export default function EditCustomerPage({ params }: EditCustomerPageProps) {
  ...
  useEffect(() => {
    if (customerData) {
      setFormData({
        ...
        provinceState: customerData.provinceState || '',
        postalZip: customerData.postalZip || '',
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: any) => {
      const response = await apiClient.put(`/api/v1/customers/${params.id}`, data);
      return response;
    },
    onSuccess: () => {
        ...
        <div className="space-y-6">
          ...
          <div className="flex justify-end gap-3">
            <Button type="button" variant="outline" asChild>
              <Link href={`/dashboard/customers/${params.id}`}>Cancel</Link>
            </Button>
